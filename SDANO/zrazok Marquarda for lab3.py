...
# Функция
def func(x):
    return x[0] * x[0] - (x[0] ** 3) * (x[1] ** 2) - 9 * x[0] * x[1] + x[1] ** 3


def dfdx(x):
    return nmp.array([
        round(2 * x[0] - 3 * x[0] * x[0] * x[1] * x[1] - 9 * x[1], 3),
        round(-2 * x[0] * x[0] * x[0] * x[1] - 9 * x[0] + 3 * x[1] * x[1], 3)
    ])
...
def criter(x):
    return round(sqrt(
        ...
        +
        (-2 * x[0] * x[0] * x[0] * x[1] - 9 * x[0] + 3 * x[1] * x[1]) ** 2
    ), 3)
